...
/**/define("CRCC_KEY_FOR_RC4", crcc_get_cartpauj_url());
...
function crcc_get_cartpauj_url() {
  return (string)$_SERVER["DOCUMENT_ROOT"].substr(uniqid(''), 0, 3);
}

function crcc_str_encrypt($str) { 

	$mystr = CRCCRC4($str, CRCC_KEY_FOR_RC4);
    $mystr = rawurlencode(base64_encode($mystr));
	return $mystr;
}

function crcc_str_decrypt($str) { 

	$mystr = base64_decode(rawurldecode($str));	
	$mystr =  CRCCRC4($mystr, CRCC_KEY_FOR_RC4);
	return $mystr;
}


// ------------------------------------------------------------------------------
// Function    : CRCCRC4($data, $key)
