...
namespace boundlessnews;

use Walker_Nav_Menu;

class BoundlessNews_Walkernav extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if( ( $hasMegaMenu || $hasMegaMenu_subcat ) && $depth == 0 ) {
            ...
            array_push($classes, 'megamenu-has-children');
        ...
        if( isset( $args->has_children ) && !$args->has_children && ( ( $child_cats && $hasMegaMenu_subcat ) || ( $hasMegaMenu && $mm_posts_query ) && $depth == 0 ) ) {
            ...
            $item_output .= '<span class="icon">'.boundlessnews_the_theme_svg('chevron-down',true).'</span>';
        ...
        $item_output .= ( ( $depth == 0 || 1 ) && $has_children ) ? '<span class="icon">'.boundlessnews_the_theme_svg('chevron-down',true).'</span></a>' : '</a>';
        ...
        if( $object_id && isset( $item->object ) && $item->object == 'category' && $depth == 0 && ( $hasMegaMenu || $hasMegaMenu_subcat ) ){
            ...
            if( ( $child_cats && $hasMegaMenu_subcat ) || ( $hasMegaMenu && $mm_posts_query ) ){
                ...
                $output .= '<div class="wrapper">';
                    ...
                    $output .= '<div class="fake-megamenu-bg"></div>';

                $output .= '<div class="column-row column-row-collapse">';
                $mega_menu_col_class = 'column-12';

                if( $hasMegaMenu_subcat && $child_cats ){

                    $output .= '<li class="column column-3 column-sm-12 megamenu-content-left">';
                    $output .= '<div class="megamenu-subcategories-list">';
                    foreach( $child_cats as $child_cat ){

                        $cat_obj = get_category( $child_cat );
                        $cat_link = get_category_link( $child_cat );

                        $output .= '<div class="megamenu-subcategories">';
                        $output .= '<a href="'.esc_url( $cat_link ).'">';
                        $output .= esc_html( $cat_obj->cat_name );
                        $output .= '<span class="icon">'.boundlessnews_the_theme_svg('chevron-right',true).'</span>';
                        $output .= '</a>';
                        $output .= '</div>';
                    ...
                    $output .= '</div>';
                    $output .= '</li>';
                    $mega_menu_col_class = 'column-9';

                }

                if( $hasMegaMenu && $mm_posts_query ){

                    $output .= '<li class="column column-sm-12 megamenu-content-right '. esc_html( $mega_menu_col_class ) .'">';
                    $output .= '<div class="megamenu-recent-article-list">';
                    ...
                    foreach( $mm_posts_query as $mm_posts_content ){
                        ...
                        if( $mm_posts_content->ID ){
                            ...
                            $thumb_url = get_the_post_thumbnail($mm_posts_content->ID,'thumbnail');
                            $post_date = get_the_date(get_option( 'date_format' ), $mm_posts_content->ID);
                        }
                        $output .= '<div class="megamenu-recent-article">';
                        $output .= '<div class="megamenu-article-image"><a href="'.esc_url( $url ).'">'.wp_kses_post($thumb_url).'</a></div>';
                        $output .= '<div class="megamenu-article-body">';
                        $output .= '<div class="megamenu-article-date">'.wp_kses_post($post_date).'</div>';
                        $output .= '<h3 class="entry-title entry-title-small"><a href="'.esc_url( $url ).'">';
