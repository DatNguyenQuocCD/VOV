<?php
if ( !class_exists('BoundlessNews_Getting_started') ):

    class BoundlessNews_Getting_started
    {   

        function __construct()
        {	

	        add_action( 'wp_ajax_boundlessnews_install_plugins', array( $this, 'boundlessnews_install_plugins' ) );
        ...
        public static function boundlessnews_plugin_status($plugin_class, $plugin_folder, $plugin_file){

            if( class_exists( $plugin_class ) ){

                return array('status' => 'active','string' => esc_html__('Deactivate','boundlessnews') );
                ...
                if( file_exists( $path ) ) {
                    ...
                    return array('status' => 'deactivate','string' => esc_html__('Activate','boundlessnews') );
                }else{

                    return array('status' => 'not-install','string' => esc_html__('Install & Active','boundlessnews') );
        ...
        public static function boundlessnews_recommended_plugins(){
        ...
        public function boundlessnews_install_plugins(){
            
            $nonce = isset( $_POST["_wpnonce"] ) ? sanitize_text_field( wp_unslash( $_POST["_wpnonce"] ) ) : '';

            if ( ! current_user_can('install_plugins') ) {
                wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'boundlessnews' ) );
            }

            // Check our nonce, if they don't match then bounce!
            if (! wp_verify_nonce( $nonce, 'boundlessnews_ajax_nonce' )) {

                wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'boundlessnews') );
            ...
            if( isset( $_POST['single'] ) ){
                ...
                $plugin_lists = $this->boundlessnews_recommended_plugins();
            ...
            foreach( $plugin_lists as $key => $plugin ){
                ...
                if( isset( $_POST['single'] ) ){
                    ...
                    $pluginstatus   = $this->boundlessnews_plugin_status( $plugin['class'],$key,$plugin['PluginFile'] );
                ...
                if( isset( $_POST['single'] ) && $PluginStatus == 'active' ) {
                    ...
                    esc_html_e('Deactivated' , 'boundlessnews');
                    die();
                    ...
                    if( isset( $_POST['single'] ) ) {
                        ...
                        esc_html_e('Activated' , 'boundlessnews');
                        die();
                    ...
                    if( file_exists($plugin_dir) ) {
                        ...
                        if( isset( $_POST['single'] ) ) {
                            esc_html_e('Installed & Activated' , 'boundlessnews');
                            die();
                        }
                    ...
                    if( isset( $_POST['single'] ) ) {
                        ...
                        esc_html_e('Failed' , 'boundlessnews');
                        die();
            ...
            if( !isset( $_POST['single'] ) ) {
                echo esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=boundlessnews-about' );
            }
    ...
    new BoundlessNews_Getting_started();
