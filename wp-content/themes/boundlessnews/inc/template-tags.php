 ...
 * @since 1.0.0
 */
if( !function_exists('boundlessnews_site_logo') ):
     ...
     */
    function boundlessnews_site_logo( $args = array(), $echo = true ){
        ...
        /**
         * Filters the arguments for `boundlessnews_site_logo()`.
         *
         * @param array $args Parsed arguments.
         * @param array $defaults Function's default arguments.
         */
        $args = apply_filters('boundlessnews_site_logo_args', $args, $defaults);
        if ( has_custom_logo() ) {
         ...
         */
        $html = apply_filters('boundlessnews_site_logo', $html, $args, $classname, $contents);
        if (!$echo) {
            return $html;
        }
...
if( !function_exists('boundlessnews_site_description') ):
     ...
     */
    function boundlessnews_site_description($echo = true){
         ...
         *
         */
        $html = apply_filters('boundlessnews_site_description', $html, $description, $wrapper);
        if (!$echo) {
            return $html;
        }
        echo $html; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
if( !function_exists('boundlessnews_posted_on') ):

    /**
     * Prints HTML with meta information for the current post-date/time.
     */
    function boundlessnews_posted_on($icon = false){

        $boundlessnews_default = boundlessnews_get_default_theme_options();
        $post_date_format = get_theme_mod( 'post_date_format',$boundlessnews_default['post_date_format'] );
        $ed_post_date = absint( get_theme_mod( 'ed_post_date',$boundlessnews_default['ed_post_date'] ) );
        ...
        if( $ed_post_date ){
            ...
            if( $post_date_format == 'time-ago' ){
                ...
                $time_string = human_time_diff( get_the_date('U'), current_time('timestamp') ).esc_html__(' Ago','boundlessnews');
...
if( !function_exists('boundlessnews_entry_footer') ):

    /**
     * Prints HTML with meta information for the categories, tags and comments.
     */
    function boundlessnews_entry_footer( $cats = true, $tags = true, $edits = true, $text = true, $icon = true ){

        $boundlessnews_default = boundlessnews_get_default_theme_options();
        $ed_post_category = absint( get_theme_mod( 'ed_post_category',$boundlessnews_default['ed_post_category'] ) );
        $ed_post_tags = absint( get_theme_mod( 'ed_post_tags',$boundlessnews_default['ed_post_tags'] ) );
                ...
                edit_post_link(
                    sprintf(
                        ...
                        /* translators: %s: Name of current post. Only visible to screen readers */
                            __('Edit <span class="screen-reader-text">%s</span>', 'boundlessnews'),
                            array(
                                'span' => array(
                                    'class' => array(),
                                ),
...
if( !function_exists('boundlessnews_is_comment_by_post_author') ):
     ...
     * @return bool
     */
    function boundlessnews_is_comment_by_post_author($comment = null){
            ...
            <div class="twp-banner-details">
                <header class="page-header">
                    <h1 class="page-title">
                        ...
                        printf( esc_html__( 'Search Results for: %s', 'boundlessnews' ), '<span>' . get_search_query() . '</span>' );
                        ?>
                    </h1>
                </header><!-- .page-header -->
            </div>
